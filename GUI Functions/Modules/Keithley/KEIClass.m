classdef KEIClass < ModuleClass
    ...
    methods (Access = public)
        ...
        function obj = KEIClass()
            ...
            %Define settings
            obj.settings{1} = SettingClass(obj,'front_rear_connect','Front/Rear?','Rear',[],...
                @obj.set_connectors_front_rear,{'Front','Rear'});
            obj.settings{end+1} = SettingClass(obj,'source_type','Source Type','Voltage',[],...
                @obj.set_source_type,{'Voltage','Current'});
            obj.settings{end+1} = SettingClass(obj,'4wire_sense','4-wire sensing?',0,...
                [], @obj.set_4wire_sense);
    ...
    % Output function
     ...
     % Source functions   
        function set_connectors_front_rear(obj, type) %'volt'/'curr'
            % Set connector in front or rear
            if any(strcmpi(type, {'rear'}))
                    obj.dev.write(':ROUT:TERM REAR');
            elseif any(strcmpi(type, {'front'}))
                    obj.dev.write(':ROUT:TERM FRON');
            end 
        end    
        ...
        function set_4wire_sense(obj, status)
            % Select 4 wire sensing on/off
            if any(status == 1) || strcmpi(status,'on') 
                    obj.dev.write(':SYST:RSEN ON')
            elseif any(status == 0) || strcmpi(status,'off') 
                    obj.dev.write(':SYST:RSEN OFF')
            end
        end
